...
public class GetResource extends Request 
{
    ...
    @Override
     public void Encode(ByteList bytes) throws Exception
     {
         ...
         bytes.update();
         int lengthinBytes = (bytes.getCurrentWritePosition() - lengthPos - 2);  bytes.update();
     ...
     public void Decode(ByteList bytes) throws ApplicationException, Exception
     {
         ...
         GameId = bytes.GetInt16();  bytes.update();
         GetResourceType = PossibleResourceType.convert(bytes.GetByte());  bytes.update();
